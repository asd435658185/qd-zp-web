 ...
 *
 */
@Component(value="evaluateSituationSelfMethod")
public class EvaluateSituationSelfMethod extends EvaluateSituationMethodStrategy{
	...
	public List<? super EvaluateSituationBaseDto> getEvaluateSituations(Long schoolId, Long classId, Long operateId,Long bagId) {
		...
		for(ZpEvaluateInfoEntity entity:entityList){
			...
			if(result!=null){
				List<ZpEvaluateDetailEntity> detailList = zpEvaluateDetailService.findRegularResultDetailList(result.getId(), EvaluateMethod.SELF,firstLeafDegree.getId());
